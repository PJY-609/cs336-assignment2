...
def worker(job_path):
    ...
    try:
        ...
        def inputs(length):
            ...
            qkv = tuple(torch.randn(1, length, d, device='cuda', dtype=dtype, generator=generator,
                                    requires_grad=True) for _ in range(3))
            do = torch.randn(1, length, d, device='cuda', dtype=dtype, generator=generator)
            return qkv, do
        ...
        def validate(tile):
            ...
            torch.cuda.synchronize()
            return dict(sequence_length=length, atol=tolerance, rtol=tolerance,
